...
const SUPABASE_KEY = 'sb_publishable_9OyB7n3foIalMmDxq1-_PA_s3xYfcJS'; // 사용 목적에 적합한 API 키
...
async function updateData() {
  ...
  const { data: hData, error: hError } = await supabase
    ...
    .update({
      ...
      prices: HANEUL_PRICES
    })
    .eq('name', '하늘강아지')
    .select();

  if (hError) {
    console.error('하늘강아지 업데이트 실패:', hError);
  } else if (hData && hData.length > 0) {
    console.log('하늘강아지 업데이트 성공:', hData[0].name);
  } else {
    console.log('하늘강아지 업데이트 결과 없음 (권한 또는 매칭 실패)');
  }
  ...
  const { data: pData, error: pError } = await supabase
    ...
    .update({
      ...
      prices: PATRASCHE_PRICES
    })
    .eq('name', '파트라슈')
    .select();

  if (pError) {
    console.error('파트라슈 업데이트 실패:', pError);
  } else if (pData && pData.length > 0) {
    console.log('파트라슈 업데이트 성공:', pData[0].name);
  } else {
    console.log('파트라슈 업데이트 결과 없음 (권한 또는 매칭 실패)');
  }
  ...
  if (iError) {
    console.error('아이헤븐 업데이트 실패:', iError);
  } else if (iData && iData.length > 0) {
    console.log('아이헤븐 업데이트 성공:', iData[0].name);
  } else {
    console.log('아이헤븐 업데이트 결과 없음 (권한 또는 매칭 실패)');
  }

  console.log('장례식장 데이터 업데이트 작업 완료!');
}
