...
$allFiles = $userFiles + ($userFiles | ForEach-Object { "onsil/$_" }) + $adminFiles + ($adminFiles | ForEach-Object { "onsil/$_" })
...
function Get-Header($type, $activePage) {
    if ($type -eq "admin") {
        $res_bold = if ($activePage -like "*reservations*") { "font-bold text-brand bg-brand/5" } else { "font-medium text-brand/70 hover:text-brand hover:bg-brand/5" }
        $part_bold = if ($activePage -like "*partners*") { "font-bold text-brand bg-brand/5" } else { "font-medium text-brand/70 hover:text-brand hover:bg-brand/5" }
        $home_bold = if ($activePage -like "*funeral_homes*") { "font-bold text-brand bg-brand/5" } else { "font-medium text-brand/70 hover:text-brand hover:bg-brand/5" }
        return @"
    <!-- Header -->
...
foreach ($fpath in $allFiles) {
    if (Test-Path $fpath) {
        # Read as bytes to avoid encoding corruption in PowerShell
        $bytes = [System.IO.File]::ReadAllBytes((Get-Item $fpath).FullName)
        # Try to detect if it's already UTF8 or something else
        # For simplicity, we'll assume it was intended to be UTF8
        $content = [System.Text.Encoding]::UTF8.GetString($bytes)
        
        $pageName = [System.IO.Path]::GetFileNameWithoutExtension($fpath)
        $type = if ($fpath -like "*admin*") { "admin" } else { "user" }
        
        # Replace Tailwind
        $content = [regex]::Replace($content, "(?s)<script>\s*tailwind\.config\s*=\s*\{.*?\}(;?)\s*</script>", $new_tailwind)
        
        # Replace Header
        $header = Get-Header $type $pageName
        ...
        # Write back as clean UTF8 (No BOM)
        [System.IO.File]::WriteAllText((Get-Item $fpath).FullName, $content, $utf8NoBom)
        Write-Host "Fixed: $fpath"
    }
}
