        ...
        const RAW_JSON = {
            "business": [
                {
                    "uuid": "snapplug-uuid-1234-5678-abcdef123456",
                    "name": "스냅플러그",
                    "enName": "snapplug",
                    "latitude": "37.518174",
                ...
                {
                    "uuid": "4ffc97ff-2782-4467-9150-f8f4a7c030cb",
                    "name": "위드엔젤",
                    "enName": "withangel",
                    "latitude": "36.458846",
        ...
        console.log(`DATA: Loaded ${funeralHomesData.length} items`);
        ...
        function renderListItems(items) {
            ...
            items.forEach(item => {
                ...
                // REAL_DATA access safety check
                const realData = (typeof REAL_DATA !== 'undefined') ? REAL_DATA[item.name] : null;
                const defaultImage = 'images/professional_funeral_service.png';
                const image = (realData && realData.imageUrl) ? realData.imageUrl : defaultImage;

                let priceText = "상담 문의";
                if (realData && realData.prices && realData.prices.length > 0) {
                    try {
                        const firstCategory = realData.prices[0];
                ...
                const rating = (realData && realData.rating) ? realData.rating : (4.5 + Math.random() * 0.5).toFixed(1);
                const reviewCount = (realData && realData.reviewCount) ? realData.reviewCount : Math.floor(Math.random() * 100) + 10;

                let badgesHtml = '';
                if (ON_CONFIG.showPartnership && item.isAlliance) badgesHtml += '<span class="bg-[#8D7B68] text-white text-xs px-2 py-1 rounded mr-2">제휴</span>';
                            ...
                            <div>
                                <div class="flex justify-between items-start mb-1">
                                    ...
                                    <div class="flex items-center gap-1">
                                        <i class="fas fa-star text-[#C5A065] text-sm"></i>
        ...
        const REGION_CONFIG = {
            'metropolitan': { name: '서울/경기/인천', coords: [37.5200, 126.9000], zoom: 10 },
        ...
        function initLeafletMap() {
            ...
            try {
                ...
                L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', {
                    maxZoom: 19,
                    attribution: '&copy; <a href="https://www.openstreetmap.org/copyright">OpenStreetMap</a> contributors, <a href="https://www.naver.com">Naver Maps</a>'
                }).addTo(leafletMap);
        ...
        function renderMapPins(items) {
            if (!leafletMap || !markerLayer) {
                console.warn('MAP: Map or MarkerLayer not initialized');
                return;
            }
            ...
            items.forEach(item => {
                ...
                const marker = L.circleMarker([lat, lng], {
                    radius: 8,
                    fillColor: (ON_CONFIG.showPartnership && item.isAlliance) ? '#C5A065' : '#8D7B68',
                    color: '#fff',
                    weight: 2,
                    opacity: 1,
                    fillOpacity: 0.9,
        ...
        function renderRegionLabels(items) {
            if (!leafletMap || !regionLayer) return;
            ...
            Object.entries(REGION_CONFIG).forEach(([key, config]) => {
                ...
                if (count > 0) {
                    const icon = L.divIcon({
                        ...
                        html: `<div class="region-badge-inner flex flex-col items-center justify-center bg-[#8D7B68] text-white rounded-full border-2 border-white cursor-pointer shadow-lg transform transition-all" style="width: 52px; height: 52px;">
                    ...
                    L.marker(config.coords, { icon: icon }).on('click', (e) => {
                        L.DomEvent.stopPropagation(e);
                        leafletMap.setView(config.coords, config.zoom);
                    }).addTo(regionLayer);
                }
            });
        }
        ...
        function getRegion(item) {
            ...
            if (addr.includes('서울') || addr.includes('경기') || addr.includes('인천')) return 'metropolitan';
            ...
            if (lat > 37.0 && lat < 38.6 && lng > 127.6) return 'gangwon';
            if (lat < 37.0 && lng > 127.6) return 'gyeongsang';
            if (lat < 36.1 && lng < 127.6) return 'jeolla';
            if (lat < 37.1 && lng < 127.6) return 'chungcheong';
            if (lat > 36.9 && lat < 38.3 && lng > 126.1 && lng < 127.9) return 'metropolitan';
            return 'other';
        }

        function updateFilterCounts() {
            const counts = {
                'metropolitan': 0, 'gangwon': 0,
                'chungcheong': 0, 'jeolla': 0, 'gyeongsang': 0
            };

            funeralHomesData.forEach(item => {
        ...
        function filterItems() {
            ...
            const checkedRegions = Array.from(checkboxes).map(cb => cb.value);
            const allianceCheckbox = document.getElementById('filter-alliance');
            const showOnlyAlliance = allianceCheckbox ? allianceCheckbox.checked : false;

            const filteredItems = enrichedHomesData.filter(item => {
                // Alliance filter
                if (showOnlyAlliance && !item.isAlliance) return false;
        ...
        document.getElementById('filter-alliance').addEventListener('change', filterItems);
        ...
        document.getElementById('btn-zoom-in').onclick = () => leafletMap && leafletMap.zoomIn();
        document.getElementById('btn-zoom-out').onclick = () => leafletMap && leafletMap.zoomOut();
        document.getElementById('btn-zoom-reset').onclick = () => leafletMap && leafletMap.setView(MAP_INITIAL_VIEW, MAP_INITIAL_ZOOM);
        ...
        const FOUR_PAWS_PRICES = [
            {
                ...
                items: [
                    ...
                    { name: "관 장례 패키지", price: "550,000원", desc: "기본화장 + 오동나무 관 + 고급수의 + 꽃다발", longDesc: "기본 화장 + 오동나무 관 + 인견 수의/수의보 + 기본 유골함 + 작은 꽃다발", imageUrl: "images/professional_funeral_service.png" },
                    { name: "요람 장례 패키지", price: "600,000원", desc: "기본화장 + 요람관 + 고급수의 + 목화솜 이불", longDesc: "기본 화장 + 요람관 + 인견 수의 + 목화솜 이불 + 기본 유골함 + 작은 꽃다발", imageUrl: "images/professional_funeral_service.png" },
                    { name: "자연닮음 장례 패키지", price: "600,000원", desc: "기본화장 + 자연닮음 관/수의 + 한지 수목함", longDesc: "기본 화장 + 자연닮음 관 + 자연닮음 수의/수의보 + 한지 수목함 + 작은 꽃다발", imageUrl: "images/professional_funeral_service.png" },
                    { name: "ALL IN ONE 패키지", price: "1,360,000원", desc: "최고급관 + 블리스 스톤 전량 + 다담 스톤함", longDesc: "기본 화장 + 오동나무 관 + 면 수의/수의보 + 블리스 스톤(전량) + 호두나무 다담 스톤함 + 작은 꽃다발", imageUrl: "images/professional_funeral_service.png" }
                ]
            },
            {
                ...
                items: [
                    ...
                    { name: "소동물 관 장례 패키지", price: "350,000원", desc: "기본화장 + 오동나무 관 + 소동물 수의보", longDesc: "기본 화장 + 오동나무 관 + 면 수의보 + 기본 유골함 + 작은 꽃다발", imageUrl: "images/professional_funeral_service.png" }
                ]
            },
            {
        ...
        const FOUR_PAWS_PRICES_SEJONG_BUSAN = [
            {
                ...
                items: [
                    ...
                    { name: "관 장례 패키지", price: "500,000원", desc: "기본화장 + 오동나무 관 + 고급수의 + 꽃다발", longDesc: "기본 화장 + 오동나무 관 + 인견 수의/수의보 + 기본 유골함 + 작은 꽃다발", imageUrl: "images/professional_funeral_service.png" },
                    { name: "요람 장례 패키지", price: "550,000원", desc: "기본화장 + 요람관 + 고급수의 + 목화솜 이불", longDesc: "기본 화장 + 요람관 + 인견 수의 + 목화솜 이불 + 기본 유골함 + 작은 꽃다발", imageUrl: "images/professional_funeral_service.png" },
                    { name: "자연닮음 장례 패키지", price: "550,000원", desc: "기본화장 + 자연닮음 관/수의 + 한지 수목함", longDesc: "기본 화장 + 자연닮음 관 + 자연닮음 수의/수의보 + 한지 수목함 + 작은 꽃다발", imageUrl: "images/professional_funeral_service.png" },
                    { name: "ALL IN ONE 패키지", price: "1,310,000원", desc: "최고급관 + 블리스 스톤 전량 + 다담 스톤함", longDesc: "기본 화장 + 오동나무 관 + 면 수의/수의보 + 블리스 스톤(전량) + 호두나무 다담 스톤함 + 작은 꽃다발", imageUrl: "images/professional_funeral_service.png" }
                ]
            },
            {
                category: "소동물 장례 패키지 (1kg 이하 기준)",
                items: [
                    ...
                    { name: "소동물 관 장례 패키지", price: "300,000원", desc: "기본화장 + 오동나무 관 + 소동물 수의보", longDesc: "기본 화장 + 오동나무 관 + 면 수의보 + 기본 유골함 + 작은 꽃다발", imageUrl: "images/professional_funeral_service.png" }
                ]
            },
            {
                category: "추가 비용 (체중 초과 안내)",
        ...
        const REAL_DATA = {
                ...
                "prices": [
                    {
                        ...
                        "items": [
                            { "name": "기본 장례", "price": "기본 제공", "desc": "염습, 추모예식, 개별화장, 유골인도, 라운지 이용 등 포함" }
                        ]
                    },
                    {
                        "category": "수의 및 관 (선택)",
            ...
            "포포즈 화성점": {
                ...
                "imageUrl": "images/professional_funeral_service.png"
            },
            "스윗드림펫": {
                "pricingType": "categorized",
                "prices": [
                    {
                        ...
                        "items": [
                            { "name": "5kg 미만", "price": "240,000원", "desc": "단독추모, 개별화장, 종이관", "longDesc": "모든 장례는 개별적인 추모 시간과 단독 화장 절차가 포함되며 기본 종이관 안치가 제공됩니다." },
            ...
            "씨엘로펫": {
                ...
                "pricingType": "categorized",
                "prices": [{ "category": "장례 패키지", "items": [{ "name": "베이직 장례 (5kg 미만)", "price": "200,000원" }, { "name": "프리미엄 장례", "price": "450,000원" }] }, { "category": "추가 옵션", "items": [{ "name": "염습", "price": "50,000원" }, { "name": "수의", "price": "30,000원~" }, { "name": "기능성 유골함", "price": "100,000원~" }] }],
                "imageUrl": "https://d21ntoj848ohi.cloudfront.net/business-main-img/cielopet.webp"
            },
            "해늘마루": { "pricingType": "categorized", "prices": HAENEULMARU_PRICES, "address": "전남 목포시 대양동 756-10" },
            "메리온": {
            ...
            "펫메모리얼": { "pricingType": "categorized", "prices": STANDARD_PRICES, "address": "강원 횡성군 공근면 초원리 109-16", "imageUrl": "https://d21ntoj848ohi.cloudfront.net/business-main-img/petmemorial.webp" },
                ...
                "prices": [
                    {
                        "category": "장례 패키지",
                        "items": [
                            { "name": "스탠다드 장례", "price": "350,000원", "desc": "기본 유골함, 고급 보자기 등" },
                ...
                "prices": [
                    {
                        "category": "기본 장례 및 화장",
                        "items": [
                            { "name": "소형동물 기본장례 (1kg 미만)", "price": "200,000원", "desc": "염/습, 추모예식, 특수 개별화장", "longDesc": "새, 햄스터 등 1kg 미만 소형동물을 위한 장례 서비스" },
            ...
            "해피엔딩": { "pricingType": "categorized", "prices": STANDARD_PRICES, "address": "경기 광주시 초월읍 지월리 134" },

            "타임투": { "pricingType": "categorized", "prices": STANDARD_PRICES, "address": "전남 함평군 학교면 월산리 259" },
            "펫바라기 남원점": {
                "pricingType": "categorized",
                "prices": [
                    {
                        "category": "기본 화장 서비스",
                        "items": [
                            { "name": "소동물 화장비", "price": "150,000원", "desc": "햄스터, 새 등", "longDesc": "염습, 장례식장 이용, 도자기 유골함이 포함된 소동물 화장비용" },
                ...
                "prices": [
                    {
                        ...
                        "items": [
                            { "name": "1kg 미만", "price": "220,000원", "desc": "가장 작은 소형 반려동물 기본 화장", "longDesc": "1kg 미만 체급의 반려동물 기본 개별 화장 서비스 비용입니다." },
            ...
            "파트라슈": { "pricingType": "categorized", "prices": STANDARD_PRICES, "address": "부산 기장군 장안읍 좌동리 49-1" },
                ...
                "prices": [
                    {
                        ...
                        "items": [
                            { "name": "기본 장례", "price": "350,000원", "desc": "염습+수의+오동나무관+백자단지+수목장 유골함", "longDesc": "염습, 한지 수의보, 오동나무관(소형), 백자 유골 단지, 수목장용 오동나무 유골함 두 종류 모두가 기본으로 제공되는 서비스입니다." },
            ...
            "엔젤스톤": {
                "pricingType": "categorized",
                "prices": [
                    { "category": "기본 화장 비용 (체중별)", "items": [{ "name": "1kg 미만", "price": "220,000원", "desc": "가장 작은 소형 반려동물 기본 화장" }, { "name": "1kg ~ 5kg 미만", "price": "250,000원", "desc": "일반적인 소형 반려동물 화장" }, { "name": "5kg ~ 15kg 미만", "price": "280,000 ~ 300,000원", "desc": "10kg 미만 28만원, 15kg 미만 30만원" }, { "name": "15kg 이상 대형견", "price": "330,000원 부터", "desc": "이후 무게 5kg 증가시 마다 점진적 차등 요금 적용" }] },
                ...
                "prices": [
                    {
                        "category": "기본 장례 및 화장",
                        "items": [
                            { "name": "기본 화장", "price": "150,000원~", "desc": "개별화장 진행 및 참관", "longDesc": "모든 장례는 개별화장으로 진행되며 마지막 순간까지 참관 가능합니다." },
                ...
                "prices": [
                    {
                        "category": "기본 장례 및 화장",
                        "items": [
                            { "name": "반려동물 장례비용", "price": "110,000원", "desc": "장례, 화장, 유골수습 포함", "longDesc": "해피펫의 기본 장례 서비스로 장례 예식, 화장 진행 및 유골 수습 과정이 모두 포함된 통합 비용입니다." }
                        ]
                    }
                ],
            ...
            "펫로스케어": { "pricingType": "categorized", "prices": STANDARD_PRICES, "address": "경남 김해시 상동면 우계리 112-1" },
            "21그램 남양주점": {
            ...
            "펫포유": {
                ...
                "prices": [
                    {
                        "category": "장례서비스 (5kg 미만 기준)",
                        "items": [
                            {
                                "name": "기본 장례",
                                "price": "180,000원",
                                "desc": "개별화장(단독) + 기본유골함",
                                "longDesc": "보호자 참관 하에 단독으로 진행되는 개별 화장과 기본 유골함이 제공되는 필수 장례 서비스입니다. (몸무게 5~20kg 미만 1kg당 1만원, 20kg 이상 1kg당 2만원 추가)"
                            },
                            {
                                "name": "펫포유 장례",
                                "price": "200,000원",
                                "desc": "염 + 단독추모실 + 국화헌화 + 종교용품 + 개별화장 + 기본유골함",
                                "longDesc": "기본 장례에 더해 아이를 정갈하게 하는 염 과정, 프라이빗한 단독 추모실에서의 이별, 헌화 및 종교용품까지 모두 제공되는 펫포유의 대표 맞춤 장례 서비스입니다."
                            }
                        ]
                    },
                    {
                        ...
                        "items": [
                            {
                                ...
                                "price": "100,000원~",
                                "desc": "삼베 / 면",
                                "longDesc": "아이의 마지막을 따뜻하게 감싸주는 수의입니다. 재질(삼베/면)에 따라 선택 가능합니다."
                            },
                            {
                                "name": "수의보",
                                "price": "120,000원~",
                                "desc": "인견 / 명주",
                                "longDesc": "아름다운 빛깔과 고급스러운 소재(인견/명주)로 지어진 수의보입니다."
                            },
                            {
                                "name": "관",
                                "price": "100,000원~",
                                "desc": "오동나무",
                                "longDesc": "부패를 방지하고 습기에 강한 최고급 오동나무로 제작된 관입니다."
                            },
                            {
                                "name": "기능성 유골함",
                                "price": "80,000원~",
                                "desc": "부패 방지, 냄새 차단",
                                "longDesc": "외부 공기를 완벽하게 차단하여 오랜 기간 아이의 뼈를 안전하고 깨끗하게 보관할 수 있는 기능성 도자기 유골함입니다."
                            }
                        ]
                    },
                    {
                        ...
                        "items": [
                            {
                                "name": "루세떼 제작",
                                "price": "600,000원",
                                "desc": "루세떼 제작 + 유리 봉안함 (유골량 150g 기준)",
                                "longDesc": "자택 또는 다른 곳에 안치 중인 유골분도 냄새 및 부패 걱정 없이 반영구적으로 보존하실 수 있도록 영혼석(루세떼)으로 제작해 드리는 특별한 서비스입니다. (최대 238개체 기본, 몰드 추가 시 비용 발생)"
                            }
                        ]
                    }
                ],
                "address": "세종 장군면 금암리 88-1",
                "imageUrl": "https://d21ntoj848ohi.cloudfront.net/business-main-img/petforyou.webp"
            },
            ...
            "한별": { "pricingType": "categorized", "prices": STANDARD_PRICES, "address": "경남 창원시 성산구 창원대로 11" },
                ...
                "prices": [
                    {
                        ...
                        "items": [
                            { "name": "01 기본장례", "price": "250,000원", "desc": "염습, 화장, 한지수의, 추모실, 보자기, 기본유골함", "longDesc": "염습, 화장, 한지수의, 국화 헌화, 기본 유골함, 단독추모실, 고급보자기, 고급액자 포함 (10kg 초과 시 추가 비용 문의)" },
            ...
            "아이별": { "pricingType": "categorized", "prices": STANDARD_PRICES, "address": "부산 기장군 장안읍 기룡리 240" },
            ...
            "강아지펫헤븐": { "pricingType": "categorized", "prices": STANDARD_PRICES, "address": "경북 성주군 선남면 오도리 91" },
            "오수펫추모공원": {
            ...
            "로이힐즈": { "pricingType": "categorized", "prices": STANDARD_PRICES, "address": "경기 양평군 양동면 삼산리 18" },
            "강릉펫사랑": {
                "pricingType": "categorized",
                "prices": [
                    {
                ...
                "prices": [
                    {
                        "category": "패키지 장례",
                        "items": [
                            { "name": "레귤러 패키지", "price": "별도 문의", "desc": "관, 수의, 유골함이 포함된 장례입니다.", "longDesc": "우리 아이를 위한 관, 수의, 유골함이 포함된 장례입니다." },
            ...
            "아이들랜드": { "pricingType": "categorized", "prices": STANDARD_PRICES, "address": "경북 경산시 와촌면 신한리 570-2" },
            "이별공간": { "pricingType": "categorized", "prices": STANDARD_PRICES, "address": "울산 울주군 삼동면 조일리 906-4" },
            ...
            "스타펫": { "pricingType": "categorized", "prices": STANDARD_PRICES, "address": "경기 포천시 내촌면 진목리 225" },
            "별이되다": {
                "pricingType": "categorized",
                "prices": [
                    {
                ...
                "prices": [
                    {
                        ...
                        "items": [
                            { "name": "기본 장례", "price": "150,000원", "desc": "예식실이용, 화장, 참관, 기본유골함", "longDesc": "모든 장례는 반려동물 체중 5kg을 기준으로 하며 초과시 1kg당 10,000원 추가비용이 발생합니다. (VAT별도)" },
                ...
                "prices": [
                    {
                        ...
                        "items": [
                            { "name": "기본 장례", "price": "250,000원", "desc": "기본 화장 등 필수 장례 절차 지원", "longDesc": "5kg 미만 기준의 가장 기본적인 장례 및 화장 절차입니다." },
                ...
                "prices": [
                    {
                        ...
                        "items": [
                            { "name": "소동물", "price": "150,000원", "desc": "기본장례", "longDesc": "부가세 별도" },
                    ...
                    if (targetCategory && targetCategory.items) {
                        targetCategory.items.forEach(p => {
                            ...
                            if (numMatch) {
                                ...
                                // '만원' 등 단위 표기 호환성 (ex: 25만원 -> 250000)
                                if (val < 10000 && p.price.includes('만')) {
                                    val = val * 10000;
                                }
                                // "추가" 과금 및 0원 등은 기본가(최저가) 산출에서 제외
        ...
        function openDetailModal(item) {
            ...
            document.getElementById('modal-image').src = details.imageUrl || `images/professional_funeral_service.png`;

            document.getElementById('modal-rating-score').textContent = details.rating;
            document.getElementById('modal-rating-count').textContent = `(${details.reviewCount})`;
        ...
        function showServiceModal(items) {
            ...
            if (items.length === 0) {
                ...
                items.forEach(item => {
                    ...
                    const defaultImg = 'images/professional_funeral_service.png';
